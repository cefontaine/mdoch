...
config const initUcellX: int = 6;
config const initUcellY: int = 6;
config const initUcellZ: int = 6;
config const nebrTabFac: int = 8;
config const rNebrShell: real = 0.4;
config const stepAvg: int = 1000;
config const stepEquil: int = 1000;
config const stepInitlzTemp: int = 200;
config const stepLimit: int = 4000;
config const runId: int = 1;
config const stepSnap: int = 1000;
...
proc init() {
	...
	// Setup files
	outFile = new file("configsnap.out", FileAccessMode.write); 
	outFile.open();
...
proc putConfig() {
	...
	blockSize = (NDIM + 1) * 8 + 3 * 8 + nMol * NDIM * 4;
...
proc main() {
	...
	while moreCycles {
		step();
		if stepCount % stepSnap == 0 then putConfig();
